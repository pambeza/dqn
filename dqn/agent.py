...
class Agent:
    def __init__(
        ...
        self.optimizer = torch.optim.RMSprop(model.parameters())
    ...
    def experiment(self, warming_up: bool = False) -> None:
        for _ in range(self.update_frequency):
            ...
            if done:
                new_state = self._reset_state()
                self.replay_buffer.store_initial_frame(new_state[-1:])
            self.state = new_state

    def warmup(self) -> None:
        """Warm up the agent by performing random actions."""
        progress_bar = tqdm(range(0, self.warmup_steps, self.update_frequency))
    ...
    def _learn(self) -> torch.Tensor:
        ...
        current_state_q_values = outputs.gather(
    ...
    def train(self) -> None:
        """Train the agent."""
        progress_bar = tqdm(range(0, self.train_steps, self.update_frequency))
        for _ in progress_bar:
            self.experiment()
            loss = self._learn()
            progress_bar.set_postfix(
                loss=loss.item(),
                update=f"{self.learning_steps_counter - self.warmup_steps}/{self.train_steps - self.warmup_steps}",
            )
